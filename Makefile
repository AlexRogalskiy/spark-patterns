...
IMAGE ?= styled-spark-patterns
OKTETO_IMAGE ?= okteto/styled-spark-patterns
DOCKER_IMAGE ?= alexanderr/styled-spark-patterns
TAG ?= latest
...
.PHONY: helm-start
helm-start:
	helm upgrade --install backend-spark-patterns -f charts/values.yaml --create-namespace --namespace webapp charts
